...
# 远程跟踪分支
pnpm --filter @git-insight/core exec node dist/cli.js preview-merge --into main --from origin/feature/x

# 冲突溯源
pnpm --filter @git-insight/core exec node dist/cli.js conflict-blame --into main --from feature/x

# 跳过 fetch
pnpm --filter @git-insight/core exec node dist/cli.js preview-merge --into main --from feature/x --no-fetch
```
