...
3. 确认后调用宿主：优先 `vscode.lm` / 已配置 API（内嵌冲突数据）；否则走 Chat 桥：冲突数据写入临时 JSON，提示词仅引用路径  
...
3. Agent 须先 Read 本批冲突数据 JSON，再按提示词 `curl` POST 回结果；普通 Chat 把 JSON 贴回弹层。冲突过多时宿主自动分批，每批单独回传后再合并  
